...
/*   Updated: 2025/08/31 23:45:36 by gada-sil         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	exec_bin(t_tree *current_node)
{
	...
	current_node->u_define.command.cmd
		= expand_and_quotes(current_node->u_define.command.cmd,
			get_data()->env);
...
static void	exec_main_node(t_tree *current_node)
{
...
static void	exec_tree(t_tree *current_node)
{
	while (current_node)
	{
		if (current_node->type == TK_PIPE)
...
void	exorcise_manager(t_tree **tree)
{
	...
	{
		exec_main_node(current_node);
		return ;
	}
	exec_tree(current_node);
}
